...
def process_media_to_mp3(media_url, job_id, bitrate='128k', sample_rate=None):
    """Convert media to MP3 format with specified bitrate and sample rate using CUDA for decoding."""
    ...
    try:
        # Enable GPU decoding
        stream = ffmpeg.input(input_filename, hwaccel='cuda')

        # Build output options for MP3
        output_options = {
            'acodec': 'libmp3lame',
