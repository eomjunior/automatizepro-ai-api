...
def process_captioning(file_url, caption_srt, caption_type, options, job_id):
    """Process video captioning using FFmpeg with CUDA acceleration."""
    try:
...
"""
        ...
        try:
            logger.info(f"Job {job_id}: Running FFmpeg with CUDA acceleration")

            ffmpeg.input(video_path, hwaccel='cuda').output(
                output_path,
                vf=subtitle_filter,
